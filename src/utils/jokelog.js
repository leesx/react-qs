window.log = console.log = (function (originFunc) {
  return function () {
    if (window.consoleParams && window.consoleParams.hideLog) return // ������������ʾlog
    let showType
    let tempStr
    let resArgs
    let backgroundUrl
    ...
    let params = Array.prototype.slice.call(arguments) // ����arguments���飬��push����
    ...
    let iconStyle = 'line-height: ' + backgroundHeightSize + 'px; padding: ' + backgroundHeightSize / 2 + 'px 10px ' + backgroundHeightSize / 2 + 'px ' + backgroundHeightSize + 'px; color: ' + color + '; fonts-size: ' + fontSize + '; background: url(' + backgroundUrl + ') no-repeat left center; background-size: auto ' + backgroundHeightSize + 'px'
